...
## Содержание
  ...
  ### [7. Технологии](#tech)
  ### [8. Схема проекта](#schema)
  ### [9. Обеспечение качества](#quality)
  ### [10. Расчет ресурсов](#resources)
...
[Диаграмма в miro](https://miro.com/app/board/uXjVI9O5aSs=/?share_link_id=293649451803)

![Alt text](image-5.png)

#### Пояснение к схеме

Фронтенд обращается к бекенду через API-Gateway, который предоставляет доступ ко всем сервисам бекенда. API-Gateway отвечает за авторизацию пользователей. Бекенд разделен на сервисы по бизнес функциям, сервисы содержат микросервисы по определенным сущностям. Если запрос требует действий сразу в нескольких сервисах, API-Gateway берет за это ответсвенность (к примеру, при обновлении аватарки пользователя сначала пойдет запрос в File service, потом в User service, чтобы заменить на новый url). 

Auth service - сервис, который отвечает за авторизацию и аутентификацию. Использует Redis для хранения сессий пользователей и PostgreSQL для доступа к таблице User.

User service - сервис, который отвечает за действия над пользователями и их контактами. Использует PostgreSQL для доступа к таблицам User и Contact. Обращается к File service для доступа к аватаркам пользователя.

Chat service - сервис, который отвечает за действия над чатами пользователей и их сообщениями. Микросервис Chat использует PostgreSQL для доступа к таблицам Chat, Chat_user, user. Микросервис Message использует Cassandra для доступа к сообщениям и PostgreSQL для доступа к таблице User. Также сервис обращается в File service для доступа к аватаркам чатов.

File service - сервис, который отвечает за действия над файлами пользователей. Микросервис Payload предоставляет доступ к вложениям пользователей в чатах, микросервис Avatar - к аватаркам пользователей и чатов.
...
| - | - |
| Backend | Создание несколько инстансов сервисов и отслеживание их состояния через Kubernetes, автоматический перезапуск при сбоях, использование nginx для балансировки нагрузки |
| Frontend | Создание несколько инстансов и отслеживание их состояния через Kubernetes, автоматический перезапуск при сбоях, использование nginx для балансировки нагрузки |
| PostgreSQL | Master-slave репликация, резервное копирование |
| Cassandra | Репликация, резервное копирование |
| S3 | Репликация, резервное копирование |
| Nginx | Active-active |
...
#### Расчет RPS и трафика по сервисам

| Сервис          | RPS                          | Пиковый RPS                | Трафик (Гбит/с)         |
|------------------|------------------------------|-----------------------------|--------------------------|
| Auth Service  | 11,966                       | 23,933                      | 0.274                    |
| User Service  | Добавление контактов + Получение профиля = 58 + 53,966 = 54,024                       | 116 + 107,932 = 108,048                     | 0.1 + 1.1 = 1.2                      |
| Chat Service  |  Создание чатов + Получение списка чатов = 116 + 55,803 = 55,919                       | 232 + 111,606 = 111,838                     |  0.2 + 5.6 = 5.8                      |
| Message Service| Отправка сообщений + Чтение сообщений = 31,478 + 135,454 = 166,932                     | 62,957 + 270,912 = 333,869                     | 15.8 + 2,950.3 = 2,966.1                  |
| File Service   | Отправка файлов + Получение файлов = 15,789 + 56,907 = 72,696                      | 31,578 + 113,814 = 145,392                    | 2,116.5 + 2116.5 = 4,233                    |

#### Допущения на 1 CPU:
| Категория             | RPS  | RAM МБ | 
|-----------------------|------|--------|
| Легкое JSON API (Go)  | 10,000 | 10     |
| Средняя бизнес-логика | 200  | 100    |
| Тяжелая бизнес-логика | 20   | 100    |
| Nginx                 | 500  | 10     |
| Cassandra             | 100  | 100    |

#### Расчет ресурсов сервисов:
| Сервис         | RPS      | Пиковый RPS | Характер сервиса      | CPU   | RAM      | Трафик Гбит/с |
|----------------|----------|-------------|-----------------------|-------|----------|---------------|
| Auth           | 11,966   | 23,933      | Легкое JSON API       | 2     | 20 МБ    | 0.274         |
| User           | 54,024   | 108,048     | Средняя бизнес-логика | 270   | 27 ГБ    | 1.2           |
| Chat           | 55,919   | 111,838     | Тяжелая бизнес-логика | 2,796 | 279.6 ГБ | 5.8           |
| Message        | 166,932  | 333,869     | Cassandra             | 1,670 | 167 ГБ   | 2,966.1       |
| File           | 72,696   | 145,392     | Средняя бизнес-логика | 364   | 36.4 ГБ  | 4,233         |
| API Gateway    | 166,932  | 333,869     | Nginx                 | 334   | 3.34 ГБ  | 4,233         |
| Итого      | 478,469 | 956,949 | -                     | 5,436 | 513.34 ГБ | 7,239.3  |

#### Сервисы в оркестрации:
| Сервис         | CPU/r | CPU/l | RAM/r  | RAM/l  | Реплики |
|----------------|-------|-------|--------|--------|----------|
| Auth           | 2     | 3     | 20 МБ  | 30 МБ  | 3        |
| User           | 16    | 32    | 2 ГБ   | 3 ГБ   | 17       |
| Chat           | 32    | 64    | 8 ГБ   | 12 ГБ  | 88       |
| Message        | 32    | 64    | 8 ГБ   | 12 ГБ  | 53       |
| File           | 16    | 32    | 2 ГБ   | 3 ГБ   | 23       |
| API Gateway    | 16    | 32    | 1 ГБ   | 2 ГБ   | 21       |

#### Хранилища:
| Хранилище  | Объем    | RPS     | CPU   | RAM    | Серверы |
|------------|----------|---------|-------|--------|---------|
| PostgreSQL | 647.19 ПБ| 54,024  | 540   | 1.1 ТБ | 9       |
| Cassandra  | 1,340 ПБ | 166,932 | 1,670 | 6.7 ТБ | 28      |
| S3         | 100 ПБ   | 72,696  | -     | -      | Облако  |
| Redis      | 50 ТБ    | 100,000 | 1,000 | 500 ГБ | 16      |

#### Конфигурация серверов:
| Название       | Конфигурация                       | Cores | RAM  | Цена ($) | Кол-во | Всего CPU |
|----------------|------------------------------------|-------|------|----------|--------|-----------|
| kubenode-large | 2xAMD EPYC 9554P/1TB/2x25GbE      | 128   | 1 ТБ | 15,000   | 45     | 5,760     |
| kubenode-small | 1xAMD EPYC 9554P/512GB/2x25GbE    | 64    | 512ГБ| 8,000    | 15     | 960       |
| db-node        | 2xAMD EPYC 9554P/2TB/4xNVMe/25GbE | 128   | 2 ТБ | 20,000   | 53     | 6,784     |

#### Итоговая стоимость:
- Bare-metal серверы: $1,415,000 (покупка) или $169,800/мес (аренда)
- Облачное S3: $25,000/мес
