...
Фронтенд обращается к бекенду через API-Gateway, который предоставляет доступ ко всем сервисам бекенда. Бекенд разделен на сервисы по бизнес функциям, сервисы содержат микросервисы по определенным сущностям. Если запрос требует действий сразу в нескольких сервисах, API-Gateway берет за это ответсвенность (к примеру, при обновлении аватарки пользователя сначала пойдет запрос в File service, потом в User service, чтобы заменить на новый url). 
